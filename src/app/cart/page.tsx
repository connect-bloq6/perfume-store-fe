...
export default function CartPage() {
  ...
  const handleDeleteItem = (id: string) => {
    ...
    if (item) {
      showToast(`${item.name} removed from cart`, 'cart_remove');
    }
  };
  ...
  const handleDeleteSelected = () => {
    ...
    showToast(`${selectedItems.length} item(s) removed from cart`, 'cart_remove');
    setSelectedItems([]);
    setSelectAll(false);
  };
  ...
  // Don't render cart items until mounted to prevent hydration mismatch
  if (!mounted) {
    return (
          ...
          {cartItems.length > 0 ? (
            <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm mb-1" style={{ color: '#6B6B6B' }}>
                        Size: <span style={{ color: '#171717' }}>{item.size}</span>
                      </p>

                      <div className="flex items-center justify-between mt-3">
                        <span 
                          className="font-semibold"
                          style={{ fontSize: '16px', color: '#A8845E' }}
                        >
              ...
              <motion.div
                ...
                style={{ backgroundColor: '#FFFFFF', border: '1px solid #F0F0F0' }}
              >
                <h3 className="font-semibold mb-4" style={{ color: '#171717', fontSize: '18px' }}>
                  Order Summary
                </h3>
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    <span style={{ color: '#6B6B6B' }}>Items ({totalItems})</span>
                    <span style={{ color: '#171717' }}>${subtotal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span style={{ color: '#6B6B6B' }}>Shipping</span>
                    <span style={{ color: '#171717' }}>Calculated at checkout</span>
                  </div>
                  <div 
                    className="flex justify-between pt-3 mt-3 font-semibold"
                    style={{ borderTop: '1px solid #F0F0F0' }}
                  >
                    <span style={{ color: '#171717' }}>Subtotal</span>
                    <span style={{ color: '#A8845E', fontSize: '18px' }}>${subtotal.toFixed(2)}</span>
                  </div>
                </div>
              </motion.div>

export interface Product {
  id: string;
  slug: string;
  name: string;
  brand: string;
  price: number;
  originalPrice?: number;
  discount?: number;
  rating: number;
  reviewCount: number;
  soldCount: number;
  tags: string[];
  volumes: { size: string; price: number }[];
  colors: { name: string; color: string; border?: boolean }[];
  description: string;
  notes: {
    top: string;
    heart: string;
    base: string;
  };
  longevity: string;
  image: string;
  category: string;
}

export const products: Product[] = [
  {
    id: '1',
    slug: 'desert-rose-signature',
    name: 'Desert Rose Signature',
    brand: 'CALRA',
    price: 370,
    originalPrice: 440,
    discount: 34,
    rating: 4.5,
    reviewCount: 47,
    soldCount: 134,
    tags: ['Best seller', 'Floral / Oriental'],
    volumes: [
      { size: '30ml', price: 270 },
      { size: '50ml', price: 370 },
      { size: '100ml', price: 520 },
      { size: '150ml', price: 680 },
    ],
    colors: [
      { name: 'Original', color: '#F5F5F5', border: true },
      { name: 'Black', color: '#2D2D2D' },
      { name: 'Gold', color: '#C4A77D' },
      { name: 'Brown', color: '#8B7355' },
      { name: 'Dark', color: '#3D3D3D' },
    ],
    description: 'Desert Rose Signature is a captivating blend of floral and oriental notes, designed for those who appreciate timeless elegance. This luxurious fragrance opens with delicate rose petals, transitions to warm amber heart notes, and settles into a rich oud base.',
    notes: {
      top: 'Rose, Bergamot, Pink Pepper',
      heart: 'Jasmine, Amber, Patchouli',
      base: 'Oud, Vanilla, Musk',
    },
    longevity: '8-10 hours',
    image: '/images/alpha.png',
    category: 'floral',
  },
  {
    id: '7',
    slug: 'la-femme-rose',
    name: 'La Femme Rose',
    brand: 'La Perfume',
    price: 325,
    originalPrice: 390,
    discount: 17,
    rating: 4.8,
    reviewCount: 93,
    soldCount: 234,
    tags: ['Women', 'Floral / Romantic'],
    volumes: [
      { size: '30ml', price: 245 },
      { size: '50ml', price: 325 },
      { size: '100ml', price: 480 },
      { size: '150ml', price: 620 },
    ],
    colors: [
      { name: 'Rose', color: '#E8B4B8', border: false },
      { name: 'Gold', color: '#C4A77D' },
      { name: 'Blush', color: '#F5D0C5' },
    ],
    description: 'La Femme Rose is an exquisite feminine fragrance that embodies grace and sophistication. A delicate symphony of rose petals, soft peony, and warm sandalwood creates an unforgettable aura of timeless beauty.',
    notes: {
      top: 'Rose Petals, Peony, Bergamot',
      heart: 'White Jasmine, Lily of the Valley, Iris',
      base: 'Sandalwood, White Musk, Cashmere Wood',
    },
    longevity: '8-10 hours',
    image: '/images/women/Desert_rose.png',
    category: 'floral',
  },
  {
    id: '2',
    slug: 'mysterious',
    name: 'Mysterious',
    brand: 'CALRA',
    price: 375,
    originalPrice: 450,
    discount: 17,
    rating: 4.7,
    reviewCount: 62,
    soldCount: 189,
    tags: ['New Arrival', 'Woody / Spicy'],
    volumes: [
      { size: '30ml', price: 275 },
      { size: '50ml', price: 375 },
      { size: '100ml', price: 540 },
      { size: '150ml', price: 700 },
    ],
    colors: [
      { name: 'Green', color: '#2D4A3E', border: false },
      { name: 'Black', color: '#1A1A1A' },
      { name: 'Gold', color: '#C4A77D' },
    ],
    description: 'Mysterious is an enigmatic fragrance that captures the essence of midnight forests and whispered secrets. A sophisticated blend of deep woods and aromatic spices creates an aura of intrigue and allure.',
    notes: {
      top: 'Cardamom, Black Pepper, Saffron',
      heart: 'Oud, Incense, Cedar',
      base: 'Sandalwood, Vetiver, Leather',
    },
    longevity: '10-12 hours',
    image: '/images/alpha_chrome.png',
    category: 'woody',
  },
  {
    id: '8',
    slug: 'velvet-orchid',
    name: 'Velvet Orchid',
    brand: 'La Perfume',
    price: 345,
    originalPrice: 420,
    discount: 18,
    rating: 4.9,
    reviewCount: 156,
    soldCount: 289,
    tags: ['Best seller', 'Women', 'Oriental'],
    volumes: [
      { size: '30ml', price: 260 },
      { size: '50ml', price: 345 },
      { size: '100ml', price: 495 },
      { size: '150ml', price: 640 },
    ],
    colors: [
      { name: 'Coral', color: '#E8A87C', border: false },
      { name: 'Gold', color: '#C4A77D' },
      { name: 'Nude', color: '#E6C9A8' },
    ],
    description: 'Velvet Orchid is a sensuous and captivating fragrance for the modern woman. Rich orchid notes intertwined with honey and rum create an intoxicating blend that lingers beautifully throughout the day.',
    notes: {
      top: 'Honey, Rum, Mandarin',
      heart: 'Orchid, Heliotrope, Jasmine',
      base: 'Suede, Vanilla, Vetiver',
    },
    longevity: '10-12 hours',
    image: '/images/women/La_perfume.png',
    category: 'oriental',
  },
  {
    id: '3',
    slug: 'black-phoenix',
    name: 'Black Phoenix',
    brand: 'CALRA',
    price: 375,
    originalPrice: 420,
    discount: 11,
    rating: 4.8,
    reviewCount: 89,
    soldCount: 256,
    tags: ['Best seller', 'Oriental / Amber'],
    volumes: [
      { size: '30ml', price: 280 },
      { size: '50ml', price: 375 },
      { size: '100ml', price: 530 },
      { size: '150ml', price: 690 },
    ],
    colors: [
      { name: 'Black', color: '#1A1A1A', border: false },
      { name: 'Gold', color: '#C4A77D' },
      { name: 'Dark Blue', color: '#1E3A5F' },
    ],
    description: 'Black Phoenix rises from the ashes with an intense and memorable presence. This bold fragrance combines smoky undertones with rich amber notes, creating a scent that commands attention and leaves a lasting impression.',
    notes: {
      top: 'Bergamot, Cinnamon, Ginger',
      heart: 'Rose Absolute, Oud, Amber',
      base: 'Musk, Benzoin, Tonka Bean',
    },
    longevity: '12+ hours',
    image: '/images/alpha_oros.png',
    category: 'oriental',
  },
  {
    id: '9',
    slug: 'midnight-bloom',
    name: 'Midnight Bloom',
    brand: 'La Perfume',
    price: 310,
    originalPrice: 375,
    discount: 17,
    rating: 4.7,
    reviewCount: 112,
    soldCount: 198,
    tags: ['New Arrival', 'Women', 'Floral'],
    volumes: [
      { size: '30ml', price: 235 },
      { size: '50ml', price: 310 },
      { size: '100ml', price: 455 },
      { size: '150ml', price: 590 },
    ],
    colors: [
      { name: 'Pink', color: '#FFB6C1', border: false },
      { name: 'Rose Gold', color: '#B76E79' },
      { name: 'Pearl', color: '#F5F5F5' },
    ],
    description: 'Midnight Bloom captures the essence of a moonlit garden in full bloom. This enchanting fragrance weaves together night-blooming jasmine, tuberose, and soft musk for a scent that is both mysterious and feminine.',
    notes: {
      top: 'Night Jasmine, Peach, Green Notes',
      heart: 'Tuberose, Gardenia, Ylang-Ylang',
      base: 'White Musk, Amber, Cedarwood',
    },
    longevity: '8-10 hours',
    image: '/images/women/la_perfume2.png',
    category: 'floral',
  },
  {
    id: '4',
    slug: 'imperial-gold-elixir',
    name: 'Imperial Gold Elixir',
    brand: 'CALRA',
    price: 420,
    originalPrice: 500,
    discount: 16,
    rating: 4.9,
    reviewCount: 124,
    soldCount: 312,
    tags: ['Limited Edition', 'Luxury'],
    volumes: [
      { size: '30ml', price: 320 },
      { size: '50ml', price: 420 },
      { size: '100ml', price: 600 },
      { size: '150ml', price: 780 },
    ],
    colors: [
      { name: 'Gold', color: '#C4A77D', border: false },
      { name: 'Rose Gold', color: '#B76E79' },
      { name: 'Platinum', color: '#E5E4E2' },
    ],
    description: 'Imperial Gold Elixir is the epitome of luxury perfumery. Crafted with the rarest ingredients from around the world, this opulent fragrance embodies sophistication and prestige. A true masterpiece for the discerning collector.',
    notes: {
      top: 'Royal Oud, Saffron, Pink Pepper',
      heart: 'Bulgarian Rose, Iris, Jasmine Sambac',
      base: 'Ambergris, Sandalwood, White Musk',
    },
    longevity: '12+ hours',
    image: '/images/alpha_safir.png',
    category: 'luxury',
  },
  {
    id: '5',
    slug: 'wild-caramel',
    name: 'Wild Caramel',
    brand: 'La Perfume',
    price: 295,
    originalPrice: 350,
    discount: 16,
    rating: 4.6,
    reviewCount: 78,
    soldCount: 198,
    tags: ['Popular', 'Gourmand / Sweet'],
    volumes: [
      { size: '30ml', price: 220 },
      { size: '50ml', price: 295 },
      { size: '100ml', price: 420 },
      { size: '150ml', price: 560 },
    ],
    colors: [
      { name: 'Caramel', color: '#C68E4E', border: false },
      { name: 'Cream', color: '#F5E6D3' },
      { name: 'Brown', color: '#8B5A2B' },
    ],
    description: 'Wild Caramel is a delectable gourmand fragrance that wraps you in warmth and sweetness. The irresistible blend of caramelized sugar, vanilla, and soft musks creates a cozy, inviting scent perfect for any occasion.',
    notes: {
      top: 'Salted Caramel, Almond, Bergamot',
      heart: 'Vanilla Orchid, Praline, Coconut',
      base: 'Sandalwood, White Musk, Benzoin',
    },
    longevity: '6-8 hours',
    image: '/images/women/intesnse02.png',
    category: 'gourmand',
  },
  {
    id: '6',
    slug: 'midnight-collection',
    name: 'Midnight Collection',
    brand: 'CALRA',
    price: 450,
    rating: 4.8,
    reviewCount: 56,
    soldCount: 145,
    tags: ['Exclusive', 'Gift Set'],
    volumes: [
      { size: '3x30ml', price: 450 },
      { size: '3x50ml', price: 620 },
    ],
    colors: [
      { name: 'Black', color: '#1A1A1A', border: false },
      { name: 'Gold', color: '#C4A77D' },
    ],
    description: 'The Midnight Collection brings together three of our most captivating evening fragrances in one luxurious set. Perfect for gifting or for those who love variety, this collection offers a scent for every mood.',
    notes: {
      top: 'Various premium notes',
      heart: 'Exclusive blends',
      base: 'Long-lasting bases',
    },
    longevity: '8-12 hours',
    image: '/images/alpha_chrome.png',
    category: 'collection',
  },
  {
    id: '16',
    slug: 'intense-passion',
    name: 'Intense Passion',
    brand: 'La Perfume',
    price: 380,
    originalPrice: 450,
    discount: 16,
    rating: 4.8,
    reviewCount: 134,
    soldCount: 267,
    tags: ['New Arrival', 'Women', 'Intense'],
    volumes: [
      { size: '30ml', price: 280 },
      { size: '50ml', price: 380 },
      { size: '100ml', price: 540 },
      { size: '150ml', price: 700 },
    ],
    colors: [
      { name: 'Rose Gold', color: '#B76E79', border: false },
      { name: 'Gold', color: '#C4A77D' },
      { name: 'Blush', color: '#F5D0C5' },
    ],
    description: 'Intense Passion is a bold and captivating fragrance that embodies feminine strength and allure. With its rich blend of exotic florals and warm spices, this scent leaves a lasting impression that turns heads.',
    notes: {
      top: 'Pink Pepper, Bergamot, Pear',
      heart: 'Rose, Peony, Magnolia',
      base: 'Patchouli, Amber, Vanilla',
    },
    longevity: '10-12 hours',
    image: '/images/women/intense3.png',
    category: 'oriental',
  },
  // Classic Collection - Old Images
  {
    id: '10',
    slug: 'desert-rose-classic',
    name: 'Desert Rose Classic',
    brand: 'CALRA',
    price: 340,
    originalPrice: 400,
    discount: 15,
    rating: 4.6,
    reviewCount: 89,
    soldCount: 312,
    tags: ['Classic', 'Floral / Oriental'],
    volumes: [
      { size: '30ml', price: 250 },
      { size: '50ml', price: 340 },
      { size: '100ml', price: 490 },
      { size: '150ml', price: 650 },
    ],
    colors: [
      { name: 'Rose', color: '#E8B4B8', border: false },
      { name: 'Gold', color: '#C4A77D' },
      { name: 'Cream', color: '#F5E6D3' },
    ],
    description: 'The original Desert Rose that started it all. A timeless classic featuring the purest Bulgarian rose essence paired with exotic Arabian oud. This heritage fragrance has captivated hearts for generations.',
    notes: {
      top: 'Bulgarian Rose, Bergamot, Saffron',
      heart: 'Turkish Rose, Geranium, Cinnamon',
      base: 'Arabian Oud, Amber, White Musk',
    },
    longevity: '10-12 hours',
    image: '/images/Landing Page/Background/Desert Rose.png',
    category: 'floral',
  },
  {
    id: '11',
    slug: 'mysterious-noir',
    name: 'Mysterious Noir',
    brand: 'CALRA',
    price: 385,
    originalPrice: 460,
    discount: 16,
    rating: 4.8,
    reviewCount: 145,
    soldCount: 278,
    tags: ['Popular', 'Woody / Dark'],
    volumes: [
      { size: '30ml', price: 285 },
      { size: '50ml', price: 385 },
      { size: '100ml', price: 550 },
      { size: '150ml', price: 720 },
    ],
    colors: [
      { name: 'Noir', color: '#1A1A1A', border: false },
      { name: 'Green', color: '#2D4A3E' },
      { name: 'Gold', color: '#C4A77D' },
    ],
    description: 'Mysterious Noir takes you deeper into the shadows with an intensified formula. Dark vetiver, smoky incense, and rich leather create an unforgettable olfactory experience for those who dare to be different.',
    notes: {
      top: 'Black Pepper, Cardamom, Grapefruit',
      heart: 'Incense, Oud, Labdanum',
      base: 'Vetiver, Leather, Dark Amber',
    },
    longevity: '12+ hours',
    image: '/images/Landing Page/Background/Mysterious.png',
    category: 'woody',
  },
  {
    id: '12',
    slug: 'black-phoenix-intense',
    name: 'Black Phoenix Intense',
    brand: 'CALRA',
    price: 395,
    originalPrice: 480,
    discount: 18,
    rating: 4.9,
    reviewCount: 167,
    soldCount: 345,
    tags: ['Best seller', 'Oriental / Intense'],
    volumes: [
      { size: '30ml', price: 295 },
      { size: '50ml', price: 395 },
      { size: '100ml', price: 565 },
      { size: '150ml', price: 740 },
    ],
    colors: [
      { name: 'Black', color: '#1A1A1A', border: false },
      { name: 'Red', color: '#8B0000' },
      { name: 'Gold', color: '#C4A77D' },
    ],
    description: 'The intensified version of our iconic Black Phoenix. Deeper, richer, and more powerful - this fragrance rises with even more presence. Perfect for those who want to make an unforgettable impression.',
    notes: {
      top: 'Saffron, Cinnamon, Blood Orange',
      heart: 'Rose Absolute, Oud Assafi, Amber',
      base: 'Musk, Benzoin, Castoreum',
    },
    longevity: '14+ hours',
    image: '/images/Landing Page/Background/Black_Phoenix.png',
    category: 'oriental',
  },
  {
    id: '13',
    slug: 'ocean-blue',
    name: 'Ocean Blue',
    brand: 'CALRA',
    price: 320,
    originalPrice: 380,
    discount: 16,
    rating: 4.5,
    reviewCount: 98,
    soldCount: 234,
    tags: ['Fresh', 'Aquatic / Marine'],
    volumes: [
      { size: '30ml', price: 240 },
      { size: '50ml', price: 320 },
      { size: '100ml', price: 460 },
      { size: '150ml', price: 600 },
    ],
    colors: [
      { name: 'Blue', color: '#1E3A5F', border: false },
      { name: 'Silver', color: '#C0C0C0' },
      { name: 'White', color: '#F5F5F5' },
    ],
    description: 'Dive into the refreshing depths of Ocean Blue. This invigorating fragrance captures the essence of crystal-clear waters, sea breeze, and sun-warmed skin. Perfect for the modern adventurer.',
    notes: {
      top: 'Sea Salt, Bergamot, Grapefruit',
      heart: 'Marine Accord, Lavender, Geranium',
      base: 'Driftwood, White Musk, Ambergris',
    },
    longevity: '8-10 hours',
    image: '/images/Landing Page/Background/Blue.png',
    category: 'fresh',
  },
  {
    id: '14',
    slug: 'golden-essence',
    name: 'Golden Essence',
    brand: 'CALRA',
    price: 365,
    originalPrice: 430,
    discount: 15,
    rating: 4.7,
    reviewCount: 112,
    soldCount: 198,
    tags: ['Unisex', 'Oriental / Warm'],
    volumes: [
      { size: '30ml', price: 270 },
      { size: '50ml', price: 365 },
      { size: '100ml', price: 520 },
      { size: '150ml', price: 680 },
    ],
    colors: [
      { name: 'Gold', color: '#C4A77D', border: false },
      { name: 'Amber', color: '#FFBF00' },
      { name: 'Bronze', color: '#CD7F32' },
    ],
    description: 'Golden Essence embodies pure luxury in a bottle. Warm amber, precious woods, and a touch of vanilla create a sophisticated scent that wraps you in golden light. Timeless elegance for any occasion.',
    notes: {
      top: 'Honey, Bergamot, Pink Pepper',
      heart: 'Amber, Jasmine, Ylang-Ylang',
      base: 'Sandalwood, Vanilla, Musk',
    },
    longevity: '10-12 hours',
    image: '/images/Landing Page/Background/Single.png',
    category: 'oriental',
  },
  {
    id: '15',
    slug: 'emerald-garden',
    name: 'Emerald Garden',
    brand: 'CALRA',
    price: 335,
    originalPrice: 395,
    discount: 15,
    rating: 4.6,
    reviewCount: 87,
    soldCount: 176,
    tags: ['New Arrival', 'Green / Fresh'],
    volumes: [
      { size: '30ml', price: 250 },
      { size: '50ml', price: 335 },
      { size: '100ml', price: 480 },
      { size: '150ml', price: 630 },
    ],
    colors: [
      { name: 'Green', color: '#2D4A3E', border: false },
      { name: 'Emerald', color: '#50C878' },
      { name: 'Gold', color: '#C4A77D' },
    ],
    description: 'Step into a lush emerald garden with this verdant fragrance. Fresh green notes dance with delicate florals and earthy vetiver, creating a natural and refreshing scent that evokes spring mornings.',
    notes: {
      top: 'Green Tea, Galbanum, Mint',
      heart: 'Lily of the Valley, Violet Leaf, Iris',
      base: 'Vetiver, Cedar, White Musk',
    },
    longevity: '8-10 hours',
    image: '/images/carla-green.png',
    category: 'fresh',
  },
];

export function getProductBySlug(slug: string): Product | undefined {
  return products.find(p => p.slug === slug);
}

export function getRelatedProducts(currentSlug: string, limit: number = 4): Product[] {
  return products.filter(p => p.slug !== currentSlug).slice(0, limit);
}

export const reviews = [
  {
    id: 1,
    name: 'Sarah Mitchell',
    initials: 'SM',
    rating: 5,
    date: '2 days ago',
    text: 'Absolutely stunning fragrance! This has become my signature scent. The blend of notes is perfectly balanced, and it lasts all day. I\'ve received so many compliments. Highly recommend!',
    helpful: 12,
    color: '#C4A77D',
  },
  {
    id: 2,
    name: 'James Davidson',
    initials: 'JD',
    rating: 4,
    date: '1 week ago',
    text: 'Great fragrance with excellent longevity. The base is rich without being overpowering. Perfect for evening wear. Only reason for 4 stars is the price point, but you\'re definitely getting quality.',
    helpful: 8,
    color: '#7B9E89',
  },
  {
    id: 3,
    name: 'Emily Carter',
    initials: 'EC',
    rating: 5,
    date: '2 weeks ago',
    text: 'This perfume is luxurious and elegant. The top notes are beautiful and the heart adds warmth. The packaging is also gorgeous - makes for a perfect gift. Will definitely be purchasing again!',
    helpful: 15,
    color: '#9B8AA6',
  },
  {
    id: 4,
    name: 'Michael Rodriguez',
    initials: 'MR',
    rating: 4,
    date: '3 weeks ago',
    text: 'Sophisticated scent that\'s perfect for special occasions. The projection is good without being overwhelming. My partner loves it! Shipping was fast and the customer service was excellent.',
    helpful: 6,
    color: '#8B9EAA',
  },
];

export const ratingBreakdown = [
  { stars: 5, count: 28 },
  { stars: 4, count: 12 },
  { stars: 3, count: 5 },
  { stars: 2, count: 1 },
  { stars: 1, count: 1 },
];

